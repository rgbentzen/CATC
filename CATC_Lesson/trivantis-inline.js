...
function ObjInlineActionTogglePlay( ) {
  ...
  {
     var strEvent = 'play';
	 if( this.plyVer == 8)  strEvent = 'playpause';
     if(! this.flsPlayer ) this.setFlashPlayer()
     if( this.autoPlay ) this.autoPlay = false;
	 else this.autoPlay = true;
     this.flsPlayer.sendEvent(strEvent);
  }
  else if( this.iType == 'flash' ) 
  {
      if(! this.flsPlayer )
...
function ObjInlinePause() {
  if( this.iType == 'flashvid' ) 
  {
     ...
     if( this.autoPlay )
     {  
        if( this.plyVer == 8 ) this.flsPlayer.sendEvent('PLAYPAUSE');
		else this.flsPlayer.sendEvent('PLAY');
     }
	 this.autoPlay = false;
  }
  else if( this.iType == 'flash' ) 

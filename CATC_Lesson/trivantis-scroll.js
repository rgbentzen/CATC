...
function ObjScroll(n,x,y,w,h,v,z,c,f,d) {
  this.name=n
  this.x=x
  this.y=y
  this.w=w
...
function ObjScrollBuild() {
  if( this.useIFrame ) {
    ...
    this.divStart='<' + this.divTag + ' id="'+this.name+'"><a name="'+this.name+'anc">\n'
...
function ObjScrollLoad(url) {
  ...
  if (this.useIFrame ) {
    ...
    var dY = this.h
    if( is.ns5 ) { dX -= 4; dY -= 4; }
    this.objLyr.write( '<iframe src=' + url + ' width=' + dX + ' height=' + dY + ' /></iframe>' )
  }
...
function ObjScrollTestActive() {
  ...
  for (var i=0;i<ObjScroll.list.length;i++) {
    var s=ObjScroll.list[i]
    if (s.activated && (s.barV.dragActive||s.window.contentlyr.slideActive)) return true
  }
  return false
}
...
function ObjScrollWActivate(w,h) {
  if( !is.ns4 && !this.scrollFrame ) return;
  if (!this.activated) {
    this.objLyr=new ObjLayer(this.name)
  ...
  if (!is.ns4 && this.scrollFrame.document.body.innerHTML) document.all[this.name+'Co'].innerHTML=this.scrollFrame.document.body.innerHTML
...
function ObjScrollWWriteContent(doc) {
  if (!is.ns4) doc.write(buildCSS('content',0,0,this.w-2*this.window.border))
}
...
function ObjScrollBActivate() {
  ...
  if (is.ns4) this.objLyrc.ele.captureEvents(Event.MOUSEDOWN | Event.MOUSEMOVE | Event.MOUSEUP)
...
function ObjScrollBIActivate() {
  ...
  this.clyr.show()
  if (is.ns4) {
    this.clyr.ele.captureEvents(Event.MOUSEDOWN | Event.MOUSEUP)
    this.clyr.ele.onmousedown = new Function(this.obj+".down(); return false;")
    this.clyr.ele.onmouseup = new Function(this.obj+".up(); return false;")
  }
  else {
